...
The scripts used to produce `docs/` and `reference/`, and the remake's asset pipeline.
Python packages: `pip install -r tools/requirements.txt` (Pillow, pefile), Python 3.10+.

- **Decompilation:** `pefile`, Ghidra 12 (`analyzeHeadless`); `capstone` only for manual disassembly.
- **Remake pipeline:** Pillow, `ffmpeg` for the sounds (`oggenc` optional, a better Vorbis
  encoder for Defold's music). Without ffmpeg the Godot target falls back to `afconvert` or
  copies the file with a warning; the Defold target fails on non-PCM wavs.
...
| `postprocess_decomp.py MODULE_DIR DECOMP_DIR OUT_DIR [--base 0x20000000]` | substitutes runtime function names, string constants, and hex-float comments |
...
| `dump_types.py MODULE_DIR [--base 0x20000000] [types…]` | field types of every Blitz `Type` from the `BBObjType` descriptors in the module → `docs/data/blitz_types.txt` |
| `hud_layout.py Data/env.b3d [--json OUT]` | projects the HUD panel's quads into 800×600 coordinates → `docs/data/hud_layout.json` |
| `build_android_template.sh GODOT_SRC [OUT_DIR]` | builds the trimmed arm64 release template for the "Android Custom" Godot preset (`make android-template`, see [godot/README.md](../godot/README.md#builds-mobile-and-web)) |
| `simulate_path.py enemy_paths.json [--raids-json raids.json] [--speed S…] [--hz N] [--json OUT]` | simulates an enemy moving along a path (ticks/seconds) → `docs/data/path_times.json` |
...
python3 tools/build_assets.py --target {godot,defold,all} [--data-dir Data] [--import-dir build/import] \
                              [--godot godot] [--defold defold] [--force] [--no-skin] [--embed]
...
| `md2togltf.py IN.md2 [SKIN] OUT.glb --data-dir Data --textures-dir … [--embed]` | MD2 → glb with morph targets (frame k at t = k) + `OUT.md2.json` (bbox) |
...
| `targets/defold/` | the Defold exporter (`export.py`, `models.py`, `materials.py`, `hud.py`, `hud_layout.py` — the wide-screen HUD anchor rules, `icons.py`) |
| `tests/` | `python3 -m unittest discover -s tools/tests -v` — checks for the parser, conversion, and export (needs Pillow; the conversion/export checks skip without the unpacked data or a completed `build_assets.py` run) |
...
The output goes to `reference/decomp/` (483 functions) and `reference/module/`. `reference/` is
gitignored (decompiled proprietary code): a fresh clone does not have it, run the pipeline above.
