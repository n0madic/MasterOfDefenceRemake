"""Conversion checks on the generated godot/assets (run tools/convert_all.py first)."""
from __future__ import annotations

import json
import unittest

from testpaths import DATA_DIR, GODOT_DIR, DOCS_DATA
...
class ManifestTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        path = GODOT_DIR / "assets" / "manifest.json"
        if not path.exists():
...
class ManifestTests(ManifestTestCase):
    ...
    def test_md2_glb_morph_targets(self) -> None:
        ...
        doc, blob = read_glb(GODOT_DIR / info["glb"])
...
class ManifestAndDataTests(ManifestTestCase):
    def test_glb_node_names_and_animation_lengths(self) -> None:
        for key, info in self.manifest["b3d"].items():
            doc, _ = read_glb(GODOT_DIR / info["glb"])
            ...
            sidecar = json.loads((GODOT_DIR / info["glb"]).with_suffix("").with_suffix(".b3d.json").read_text())
...
class GlbTests(unittest.TestCase):
    def test_path_keys_equal_converted_export(self) -> None:
        ...
        for L in range(1, 7):
            doc, blob = read_glb(GODOT_DIR / "assets" / "models" / f"Location{L}" / "Path1.glb")
    ...
    def test_floor_quads_face_up_after_conversion(self) -> None:
        ...
        for L in (1, 2, 3):
            doc, blob = read_glb(GODOT_DIR / "assets" / "models" / f"Location{L}" / f"Location{L}.glb")
    ...
    def test_location3_has_separate_grass01(self) -> None:
        doc, _ = read_glb(GODOT_DIR / "assets" / "models" / "Location3" / "Location3.glb")
        names = {n["name"] for n in doc["nodes"]}
        self.assertIn("grass", names)
        self.assertIn("grass01", names)
        for L in range(1, 7):
            doc, _ = read_glb(GODOT_DIR / "assets" / "models" / f"Location{L}" / f"Location{L}.glb")
            names = {n["name"] for n in doc["nodes"]}
    ...
    def test_b3d_bones_become_skins(self) -> None:
        ...
        for model, joints in (("Towers/Balloon.glb", 2), ("Monsters/Health.glb", 8)):
            with self.subTest(model=model):
                doc, blob = read_glb(GODOT_DIR / "assets" / "models" / model)
    ...
    def test_skin_rest_pose_inverse_bind(self) -> None:
        ...
        import b3d2gltf
        doc, blob = read_glb(GODOT_DIR / "assets" / "models" / "Towers" / "Balloon.glb")
        skin = doc["skins"][0]
    ...
    def test_ogg_comment_header_is_well_formed(self) -> None:
        """Godot rejects Vorbis user comments without '='; copy_assets remuxes them away."""
    ...
    def test_colours_are_linearised_and_vertex_alpha_follows_blitz_blending(self) -> None:
        ...
        doc, blob = read_glb(GODOT_DIR / "assets" / "models" / "Location3" / "Location3.glb")
        ...
        doc, blob = read_glb(GODOT_DIR / "assets" / "models" / "Location5" / "Location5.glb")
        node = next(n for n in doc["nodes"] if n["name"] == "grass")
        prim = doc["meshes"][node["mesh"]]["primitives"][0]
        ...
        doc, blob = read_glb(GODOT_DIR / "assets" / "models" / "Menu" / "env.glb")
    ...
    def test_mirrored_nodes_get_reversed_winding(self) -> None:
        ...
        doc, blob = read_glb(GODOT_DIR / "assets" / "models" / "Location6" / "Location6.glb")
    ...
    def test_texture_clamp_flags_are_recorded_per_layer(self) -> None:
        ...
        sidecar = json.loads((GODOT_DIR / "assets" / "models" / "Location2" / "Location2.b3d.json").read_text())
        ...
        sidecar = json.loads((GODOT_DIR / "assets" / "models" / "Menu" / "env.b3d.json").read_text())
